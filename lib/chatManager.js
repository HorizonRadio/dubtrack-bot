...
function cleanChatAntiCaps(utils) {
    ...
    if(upper > 10 && (upper / (upper + lower)) > 0.5) {
        ...
        console.info('User Message: ' + utils.getMessage());
        console.info('Message matched ' + (upper / (upper + lower)) * 100 + '%');
        utils.reply.stop();
    }
}
...
function cleanChatImageSpam(utils) {
    if(!utils.hasImages())
        return (sameUserImagesCount = 0);

    if(utils.settingsManager.getImgRemoveSpamAmount() < 0)
        return;

    sameUserImagesCount += utils.getImageAmount();

    if(!MessageUtils.getLastMessageWithImage())
        return;

    if(MessageUtils.getLastMessageWithImage().getUserId() !== utils.getUserId())
        sameUserImagesCount = 0;

    if(sameUserImagesCount === parseInt(utils.settingsManager.getImgRemoveSpamAmount())) {
        safeRemoveMessage(utils, () => sameUserImagesCount = 0);
        if(!utils.botUtils.checkRole(utils.getUser(), 'resident-dj'))
            utils.timeMuteUser(
    ...
    } else if(sameUserImagesCount > utils.settingsManager.getImgRemoveSpamAmount())
        utils.bot.moderateDeleteChat(utils.getFirstMessageByUser().getId()); // silent deletion
}
...
function cleanChatAdvertisingBan(utils) {
    ...
    if(advertiseMatch) {
        ...
        safeRemoveMessage(utils);
        utils.botUtils.banUser(utils.getUser(), 'User banned. Reason: Advertising another Dubtrack/Plug.DJ room.');
        utils.reply.stop();
    }
}
