...
function cleanChatBanPhrases(utils) {
    ...
    fs.stat(banData, function(err, stat) {
        ...
        for(var key in phrases) {
            const phraseObj = phrases[key];
            if(typeof phraseObj != 'function') {
                var phrasesList;
                ...
                for(var i = 0; i < phrasesList.length; i++) {
                    ...
                    if(checkIfContains(phrase)) {
                        ...
                        if(isPermanent)
                            utils.botUtils.banUser(utils.getUser(), null, true, phraseObj.reason, banCallback)
                        else utils.botUtils.timeBan(utils.getUser(), null, phraseObj.banTime, true, phraseObj.reason, banCallback)
                        utils.reply.stop();
                        break;
                    }
                }
...
function cleanChatAdvertisingBan(utils) {
    ...
    if(advertiseMatch) {
        ...
        safeRemoveMessage(utils);
        utils.botUtils.banUser(utils.getUser(), null, true, 'advertising another DubtrackFM/Plug.DJ room');
        utils.reply.stop();
    }
}
