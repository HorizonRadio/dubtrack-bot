...
function genGameTimestampName(gameName) {
    return 'game:' + gameName + ':last';
}

// Game timestamps
// => last ran timestamp
RedisManager.prototype.setLastGameTimestamp = function(gameName, reset) {
    this.redis.set(genGameTimestampName(gameName), reset ? -1 : Date.now());
};
RedisManager.prototype.getLastGameTimestamp = function(gameName, callback) {
    this.redis.get(genGameTimestampName(gameName), processReturn(callback, function(result) {
