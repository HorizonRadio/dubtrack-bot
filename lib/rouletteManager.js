...
function end() {
    this.started = false;
    this.listedUsers.length = 0;
    this.redisManager.setLastGameTimestamp('roulette');
}
RouletteManager.prototype.start = function (duration, price, onEnd) {
    duration = duration || this.settingsManager.getRouletteDuration();
    this.currentPrice = typeof price === 'undefined' ? this.settingsManager.getRoulettePrice() : price;
    if (this.started) {
        return false;
    }
