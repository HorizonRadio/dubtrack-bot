...
var scramble_reward = (process.env.SCRAMBLE_REWARD == undefined ? 3 : process.env.SCRAMBLE_REWARD); // number
var scramble_dur = (process.env.SCRAMBLE_DURATION == undefined ? 60 : process.env.SCRAMBLE_DURATION); // time in seconds
var scramble_cd = (process.env.SCRAMBLE_COOLDOWN == undefined ? 30 : process.env.SCRAMBLE_COOLDOWN); // time in minutes
var wordnik_api_key = (process.env.WORDNIK_API_KEY); // string
...
exports.getScrambleReward = function () {
    var reward = parseInt(scramble_reward);
    return isNaN(reward) ? 3 : reward;
};

exports.getScrambleDuration = function () {
    var duration = parseInt(scramble_dur);
    return isNaN(duration) ? 60 : duration;
};

exports.getScrambleCooldown = function () {
    var cooldown = parseInt(scramble_cd);
